...
namespace _Scripts.Stage.Player
{
    public class CarrierBehaviour : AttachableNode
    {
         ...
         {
            // _carrierStatus.CurIngredient?.Throw(throwPoint.position,throwPoint.rotation,throwPoint.forward, _moveStatus.SpeedMultiplier).Forget();
            // _carrierStatus.CurIngredient?.Detach();
         }
         ...
         private void OnThrowStarted(InputAction.CallbackContext ctx)
         {
            ...
            // hold started와 canceled는 무조건 호출된다는 점에 주의...
            switch (ctx.interaction)
            {
               case HoldInteraction: break;
               case PressInteraction: ThrowServerRpc(); break;
               default: break;
            }
         }
